...
typedef struct ak_isp_blc {
    T_U16     black_level_enable;	//Ê¹ÄÜÎ»   
    T_U16     bl_r_a;				//[ 0,1023]
...
typedef struct ak_isp_blc_attr {
    T_U16       blc_mode;             //0Áª¶¯Ä£Êœ£¬1ÊÖ¶¯Ä£Êœ
    AK_ISP_BLC  m_blc;
...
typedef struct ak_isp_rgb_gamma_attr {
    ...
    T_U16   g_key[16];
    T_U16   b_key[16];
    T_U16   rgb_gamma_enable;  //Èç¹û²»Ê¹ÄÜ£¬ŸÍÊÇÒ»ÌõÖ±Ïß
} AK_ISP_RGB_GAMMA_ATTR;

typedef struct ak_isp_y_gamma_attr {
	T_U16    ygamma[129];    //10bit
	T_U16    ygamma_key[16]; //ÇúÏßµÄ¹ØŒüµã 
	T_U16    ygamma_uv_adjust_enable;
	T_U16    ygamma_uv_adjust_level;
	T_U16    ygamma_cnoise_yth1;   //YgammaÉ«²îÒÖÖÆÃÅÏÞÖµ
	T_U16    ygamma_cnoise_yth2;   //YgammaÉ«²îÒÖÖÆÃÅÏÞÖµ
	T_U16    ygamma_cnoise_slop;   
	T_U16    ygamma_cnoise_gain ;  //UVµ÷ÕûÏµÊýŒÆËã²ÎÊý
}AK_ISP_Y_GAMMA_ATTR;

typedef struct ak_isp_nr1 {
    T_U16  nr1_enable;            //Ê¹ÄÜÎ»
...
typedef struct ak_isp_s_nr1_attr {
    T_U16       nr1_mode;             //nr1 Ä£Êœ£¬×Ô¶¯»òÕßÁª¶¯Ä£Êœ
    AK_ISP_NR1  manual_nr1;
    AK_ISP_NR1  linkage_nr1[9];       //Áª¶¯²ÎÊý      
}AK_ISP_NR1_ATTR;
...
typedef struct  ak_isp_3d_nr {
    ...
    T_U16 MD_th;              	//ÔË¶¯Œì²âãÐÖµ [0-127]
    T_U16 updata_ref_y;       	//žüÐÂY
    T_U16 updata_ref_uv;		//žüÐÂuv
    T_U16 tnr_refFrame_format;	//²Î¿ŒÖ¡µÄžñÊœ
...
typedef struct ak_isp_3d_nr {
	...
	T_U32		md_th;			//[0, 65535]  ÔË¶¯Œì²âãÐÖµ [0-127]
}AK_ISP_3D_NR;
...
typedef struct  ak_isp_3d_nr_stat_info {
	T_U16		MD_stat_max;	
	T_U16		MD_stat[24][32];		//ÔË¶¯Œì²â·Ö¿éÊä³ölz0499 9_12
	T_U32		MD_level;			    //ÔË¶¯Œì²âÊä³ö    
}AK_ISP_3D_NR_STAT_INFO;

typedef struct ak_isp_gb {

    T_U16   gb_enable;        //Ê¹ÄÜÎ»
...
typedef struct ak_isp_gb_attr {
    T_U16      gb_mode;       //Ä£ÊœÑ¡Ôñ£¬ÊÖ¶¯»òÕßÁª¶¯
    AK_ISP_GB  manual_gb;
    AK_ISP_GB  linkage_gb[9];
} AK_ISP_GB_ATTR;

typedef struct ak_isp_demo_attr {
    T_U16  dm_HV_th;       //·œÏòÅÐ±ðÏµÊý
...
typedef struct ak_isp_ccm {
	T_U16  cc_enable;		   //ÑÕÉ«Ð£ÕýÊ¹ÄÜ
	T_U16  cc_cnoise_yth;	   //ÁÁ¶È¿ØÖÆÔöÒæ
	T_U16  cc_cnoise_gain;	   //ÁÁ¶È¿ØÖÆÔöÒæ
	T_U16  cc_cnoise_slop;	   //ÁÁ¶È¿ØÖÆÔöÒæ
	T_S16  ccm[3][3];		   //[-2048, 2047]    
}AK_ISP_CCM;

typedef struct ak_isp_ccm_attr {
    T_U16       cc_mode;  		//ÑÕÉ«Ð£ÕýŸØÕóÁª¶¯»òÕßÊÖ¶¯
    AK_ISP_CCM  manual_ccm; 
    AK_ISP_CCM  ccm[4]; 		//ËÄžöÁª¶¯ŸØÕó
}AK_ISP_CCM_ATTR;

typedef struct ak_isp_wdr {
    T_U16   hdr_uv_adjust_level;        //uvµ÷Õû³Ì¶È, [0,31]
    T_U16   hdr_cnoise_suppress_slop;   //ÒÖÖÆÐ±ÂÊ  
    T_U16   wdr_enable;
	...
	T_U16	area_tb1[65];	  //ÇúÏß 10bit
	T_U16	area_tb2[65];	  //ÇúÏß 10bit
	T_U16	area_tb3[65];	  //ÇúÏß 10bit
	T_U16	area_tb4[65];	  //ÇúÏß 10bit
	T_U16	area_tb5[65];	  //ÇúÏß 10bit
	T_U16	area_tb6[65];	  //ÇúÏß 10bit
    ...
    T_U16   hdr_uv_adjust_enable;       //uvµ÷ÕûÊ¹ÄÜ
    T_U16   hdr_cnoise_suppress_yth1;   //É«²ÊÔëÉùÁÁ¶ÈãÐÖµ1           
    T_U16   hdr_cnoise_suppress_yth2;   //É«²ÊÔëÉùÁÁ¶ÈãÐÖµ2
    T_U16   hdr_cnoise_suppress_gain;   //É«²îÒÖÖÆ
}AK_ISP_WDR;

typedef struct ak_isp_wdr_attr {
	T_U16 wdr_mode;              //Ä£ÊœÑ¡Ôñ£¬ÊÖ¶¯»òÕßÁª¶¯
...
typedef struct  ak_isp_contrast_ATTR {
	T_U16	cc_mode;	//Ä£ÊœÑ¡Ôñ£¬ÊÖ¶¯»òÕßÁª¶¯
...
typedef struct ak_isp_hue_attr {
	T_U16        hue_mode;		//Áª¶¯»òÕßÊÖ¶¯
	AK_ISP_HUE   manual_hue; 
	AK_ISP_HUE   hue[4];		//ËÄžöÁª¶¯²ÎÊý
}AK_ISP_HUE_ATTR;

typedef  struct ak_isp_rgb2yuv_attr {
    T_U16  mode; 				//bt601 »òÕßbt709
}AK_ISP_RGB2YUV_ATTR;
...
typedef struct ak_isp_effect_attr {
    ...
    T_S16  uv_a;    //[-256, 255]
    T_S16  uv_b;    //[-256, 255]
    T_U16  dark_margin_en;   //ºÚ±ßÊ¹ÄÜ
}AK_ISP_EFFECT_ATTR;

typedef struct ak_isp_ddpc { 
	T_U16 ddpc_enable;         //¶¯Ì¬»µµãÊ¹ÄÜÎ»
	T_U16 ddpc_th;             //10bit
	T_U16 white_dpc_enable;    //°×µãÏû³ýÊ¹ÄÜÎ»
	T_U16 black_dpc_enable;    //ºÚµãÏû³ýÊ¹ÄÜÎ»
}AK_ISP_DDPC;
 
typedef struct ak_isp_ddpc_attr {
	T_U16 ddpc_mode;            //Ä£ÊœÑ¡Ôñ£¬ÊÖ¶¯»òÕßÁª¶¯
	AK_ISP_DDPC manual_ddpc;
	AK_ISP_DDPC linkage_ddpc[9]; 
}AK_ISP_DDPC_ATTR;

typedef struct ak_isp_sdpc_attr { 
    T_U32   sdpc_enable; 		//Ÿ²Ì¬»µµãÊ¹ÄÜÎ»

    /* Ÿ²Ì¬»µµã×ø±êÖµ£¬×îŽóÎª1024žö
    ÊýŸÝžñÊœ{6h0 ,y_position[9:0],5'h0,x_position[10:0]} */
...
typedef struct ak_isp_af_stat_info {
   T_U32  af_statics[5];            //Í³ŒÆœá¹û  
}AK_ISP_AF_STAT_INFO;

typedef struct ak_isp_weight_attr {
   T_U16   zone_weight[8][16];      //ÈšÖØÏµÊý  
}AK_ISP_WEIGHT_ATTR;
...
typedef  struct ak_isp_awb_attr {   
    ...
    //awbÈíŒþ²¿·ÖÐèÒªÉèÖÃµÄ²ÎÊý
    T_U16   auto_wb_step;                 //°×Æœºâ²œ³€ŒÆËã
    T_U16   total_cnt_thresh;            //ÏñËØžöÊýãÐÖµ
    T_U16   colortemp_stable_cnt_thresh; //ÎÈ¶šÖ¡Êý£¬¶àÉÙÖ¡Ò»ÑùÈÏÎª»·Ÿ³É«ÎÂžÄ±ä
...
typedef  struct ak_isp_awb_stat_info {
    ...
    T_U32   total_B[10];
    T_U32   total_cnt[10];
    //Ÿ­ÓÉ×Ô¶¯°×ÆœºâËã·šËã³öµÄ°×ÆœºâÔöÒæÖµ
    T_U16   r_gain;
    T_U16   g_gain;
    T_U16   b_gain;
    T_S16   r_offset;
    T_S16   g_offset;
    T_S16   b_offset;
    T_U16   current_colortemp_index;     //»·Ÿ³É«ÎÂ±êŒÇ£¬ÊÇ²ÎÊýËæ»·Ÿ³±ä»¯µÄÉ«ÎÂÖž±ê¡£
    T_U16   colortemp_stable_cnt[10];         //Ã¿Ò»ÖÖÉ«ÎÂÎÈ¶šµÄÖ¡ÊýŒÆÊý
...
typedef struct ak_isp_ae_attr {
    T_U32   exp_time_max;			//ÆØ¹âÊ±ŒäµÄ×îŽóÖµ
    T_U32   exp_time_min;			//ÆØ¹âÊ±ŒäµÄ×îÐ¡Öµ
    T_U32   d_gain_max;      		//Êý×ÖÔöÒæµÄ×îŽóÖµ
    T_U32   d_gain_min;     		//Êý×ÖÔöÒæµÄ×îÐ¡Öµ
    T_U32   isp_d_gain_min;  		//ispÊý×ÖÔöÒæµÄ×îÐ¡
    T_U32   isp_d_gain_max;  		//ispÊý×ÖÔöÒæµÄ×îŽóÖµ
    T_U32   a_gain_max;     		//Ä£ÄâÔöÒæµÄ×îŽóÖµ
    T_U32   a_gain_min;      		//Ä£ÄâÔöÒæµÄ×îÐ¡Öµ     
    T_U32   exp_step;            	//ÓÃ»§ÆØ¹âµ÷Õû²œ³€
    T_U32   exp_stable_range;     	//ÎÈ¶š·¶Î§
    T_U32   target_lumiance;     	//Ä¿±êÁÁ¶È
...
typedef  struct ak_isp_ae_run_info {
    T_U8   current_calc_avg_lumi;         		//ÏÖÔÚµÄŒÆËã³öµÄÁÁ¶ÈÖµ
    T_U8   current_calc_avg_compensation_lumi; 	//Ÿ­¹ýÆØ¹â²¹³¥ºóµÄÁÁ¶ÈÖµ

    T_U8   current_darked_flag;                 //°×ÌìºÚÒ¹µÄ±êŒÇ

    T_S32  current_a_gain;                     	//Ä£ÄâÔöÒæµÄÖµ
    T_S32  current_d_gain;                    	//Êý×ÖÔöÒæµÄÖµ
    T_S32  current_isp_d_gain;                	//ispÊý×ÖÔöÒæµÄÖµ
    T_S32  current_exp_time;                	//ÆØ¹âÊ±ŒäµÄÖµ

    T_U32  current_a_gain_step;             	//ÏÖÔÚµÄÄ£ÄâÔöÒæµÄ²œ³€
    T_U32  current_d_gain_step;             	//Êý×ÖÔöÒæµÄ²œ³€
    T_U32  current_isp_d_gain_step;        		//ispÊý×ÖÔöÒæµÄ²œ³€
    T_U32  current_exp_time_step;          		//ÆØ¹âÊ±ŒäµÄ²œ³€
}AK_ISP_AE_RUN_INFO;
    
typedef struct ak_isp_aec_algo {
    T_U32  aec_status;       		//×Ô¶¯ÆØ¹â¿ØÖÆµÄ×ŽÌ¬£¬ÎÈ¶š»òÕß²»ÎÈ¶š  
    T_U32  aec_locked;      		//×Ô¶¯ÆØ¹âËø¶šÓë·ñ
    
    T_U8  exp_time_need_updata;   	//ÐèÒª±»žüÐÂ£¬ŒõÉÙiic¶ÁÐŽŽÎÊý         
    T_U8  a_gain_need_updata;    	//ÐèÒª±»žüÐÂ£¬ŒõÉÙiic¶ÁÐŽŽÎÊý
    T_U8  d_gain_need_updata;       //ÐèÒª±»žüÐÂ£¬Ö»ÓÐÐèÒªžüÐÂÊ±²ÅÐŽ
    T_U8  isp_d_gain_need_updata; 	//ÐèÒª±»žüÐÂ
    T_U8  lock_range;
}AK_ISP_AEC_ALGO;
...
typedef struct ak_isp_mask_color_attr {
    T_U8  color_type;   //0:Öž¶šÕÚµ²É«£»1£º
    T_U8  mk_alpha;
    T_U8  y_mk_color;
    T_U8  u_mk_color;
    T_U8  v_mk_color;
...
typedef int (*ISPDRV_CB_WORD_READ)(unsigned long addr, unsigned int *val);
typedef int (*ISPDRV_CB_WORD_WRITE)(unsigned long addr, unsigned int val);

   extern ISPDRV_CB_WORD_READ  cb_word_read;
   extern ISPDRV_CB_WORD_WRITE cb_word_write;
