/*
 * video.c - ISP video data processing
 *
...
#include <mach/map.h>
bool is_first_input(struct input_video_attr *input);
int get_dvp_bits_width(struct input_video_attr *input);
bool is_dual_sensor_mode(struct input_video_attr *input);
int get_input_scan_method(struct input_video_attr *input);
extern int ak_isp_awb_work(void);
extern int ak_isp_ae_work(void);
extern int ak_vb2_queue_setup(...);


/* =====================================================================
 *  BEGIN COMPAT STUBS (missing vendor SDK headers)
 * ===================================================================== */
...
/* Sensor callback shapes (as dev.c expects) */
struct sensor_cb {
    ...
    int  (*sensor_get_valid_coordinate_func)(void *arg,
                                             int *left, int *top);
...
struct input_data_format {
    int df;          /* enum ak_df */
    int data_width;  /* bits per pixel (8/10/12/16) */
};

/* AE fast struct placeholder */
...
/* RAW data header */
#define RAWDATA_HEADER_MAGIC  0x41574B52u
...
static inline void isp_set_pp_frame_ctrl_single(void *a, int b, int c)
{
    (void)a; (void)b; (void)c;
}

static inline void ak_isp_vi_start_capturing_one(void *isp)
{
    (void)isp;
}

static inline void camera_isp_resume_isp_capturing(void *isp)
{
    (void)isp;
}

/* From ak_isp_drv.h — these take NO parameters */
extern int ak_isp_ae_work(void);
extern int ak_isp_awb_work(void);

/* Corrected helper stub */
...
/*
 * defined: start streaming in irq handler in continuous mode.
 */
#define CNT_MODE_START_CHN_IN_IRQ

/* Single global v4l2_device */
static struct v4l2_device akisp_v4l2_dev;
static bool akisp_v4l2_dev_registered;

/* Pinctrl structure (copied minimally) */
struct pinctrl {
...
/* static bool akisp_v4l2_dev_registered = false; */
 ...
 */
static int set_pinctrl_vi(struct input_video_attr *input_video,
                          enum pinctrl_name pname)
{
    int ret = 0;static const char * const pnames[] = {
        "mipi0_2lane",
        "mipi0_1lane",
        "mipi1_2lane",
        "mipi1_1lane",
    ...
    if (pname < 0 || pname >= ARRAY_SIZE(pnames)) {
        pr_err("%s: invalid pname=%d\n", __func__, pname);
        return -EINVAL;
    }

    pstate = pinctrl_lookup_state(hw->pinctrl, pnames[pname]);
    if (IS_ERR(pstate)) {
        pr_err("%s: cannot find pinctrl state '%s'\n",
    ...
    if (ret)
        pr_err("%s: pinctrl_select_state(%s) failed: %d\n",
               __func__, pnames[pname], ret);
...
{
    int ret = 0;#ifdef CONFIG_MACH_AK39EV330
	struct ak_camera_dev *ak_cam = input_video_to_ak_cam(input_video);
	struct hw_attr *hw = &ak_cam->hw;
#endif
...
{
    int ret = 0;struct sensor_attr *sensor = &input_video->sensor;
	struct sensor_cb_info *sensor_cbi = sensor->sensor_cbi;
	int input_id = input_video->input_id;
...
{
    int ret = 0;struct ak_camera_dev *ak_cam = input_video_to_ak_cam(input_video);
	struct hw_attr *hw = &ak_cam->hw;
	struct sensor_attr *sensor = &input_video->sensor;
	struct sensor_cb_info *sensor_cbi = sensor->sensor_cbi;
	int interface;
...
{
    int ret = 0;struct input_video_attr *input_video = chn_to_input_video(chn);
	struct sensor_attr *sensor = &input_video->sensor;
	struct isp_attr *isp = &input_video->isp;
	struct sensor_cb_info *sensor_cbi = sensor->sensor_cbi;
...
{
    int ret = 0;struct vb2_v4l2_buffer *vb2_v4l2_b = vb2_b_to_vb2_v4l2_b(vb2_b);
...
{
    int ret = 0;struct chn_attr *chn = video_drvdata(file);
	struct input_video_attr *input_video = chn_to_input_video(chn);
	struct ak_camera_dev *ak_cam = input_video_to_ak_cam(input_video);
	struct hw_attr *hw = &ak_cam->hw;
...
{
    int ret = 0;struct v4l2_fh *vfh = file->private_data;
	struct ak_cam_fh *akfh = vfh_to_ak_cam_fh(vfh);
	struct chn_attr *chn = video_drvdata(file);
	struct input_video_attr *input_video = chn_to_input_video(chn);
	struct ak_camera_dev *ak_cam = input_video_to_ak_cam(input_video);
...
int input_video_init(struct input_video_attr *input_video)
{
	...
	pr_debug("%s %d\n", __func__, __LINE__);
	if (!akisp_v4l2_dev_registered) { ret = v4l2_device_register(dev, v4l2_dev); if (ret) { pr_err("%s: v4l2_device_register failed: %d\n", func, ret); return ret; } akisp_v4l2_dev_registered = true; pr_info("%s: registered v4l2_device %p\n", func, v4l2_dev); }
	/* register hw->v4l2_dev once so vdev->v4l2_dev is valid for video core */ if (!akisp_v4l2_dev_registered) { ret = v4l2_device_register(dev, v4l2_dev); if (ret) { pr_err("%s: v4l2_device_register failed: %d\n", func, ret); return ret; } akisp_v4l2_dev_registered = true; pr_info("%s: registered v4l2_device %p\n", func, v4l2_dev); }
...
if (akisp_v4l2_dev_registered) { v4l2_device_unregister(&hw->v4l2_dev); akisp_v4l2_dev_registered = false; pr_info("%s: unregistered v4l2_device\n", func); }
	return 0;
    return ret;
}
